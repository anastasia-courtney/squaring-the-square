...
pub fn decompose_restricted(config: &mut RestrictedConfig, plate_id: usize) -> () { //given a plate, decompose it by adding squares, then select the next plate if the plates change
    // if filling the plate with a square does not make the height greater than the size, add the square and then next plate
    if config.eligible_square(config.plates[plate_id].width) && config.plates[plate_id].height + config.plates[plate_id].width <= config.size{
        let config_backup = config.clone();
        config.vertical_extension(plate_id);
        next_plate_restricted(config);
        //undo it
        *config = config_backup;
        ////eprintln!("a- {}", config);
    }
    ...
    // if the height separating the plate from the one to it's left is less than the length, extend the left plate horizontally by adding the square
    if config.eligible_square(config.plates[plate_id - 1].height - config.plates[plate_id].height) && config.plates[plate_id - 1].height - config.plates[plate_id].height < config.plates[plate_id].width{
        config.horizontal_extension(plate_id);
        decompose_restricted(config, plate_id);
        //remove the square
        config.reverse_horizontal_extension(plate_id);
    }
    else{
    ...
    for s in 2..(std::cmp::min(config.plates[plate_id].width - 1, config.size - config.plates[plate_id].height)+1) {
        // if the square can be added to the bottom left corner, add it and then decompose the new plate)
        if config.eligible_square(s) && s != config.plates[plate_id-1].height - config.plates[plate_id].height{
            //print number of plates:
            config.add_square_quick(s, plate_id);
            next_plate_restricted(config);
            //decompose_restricted(config, plate_id + 1);
            //remove the square
            config.remove_square(plate_id);
        }
        else{
...
fn next_plate_restricted(config: &mut RestrictedConfig) -> () { //find smallest delimited plate, and decompose it
    let mut l_min : Integer = config.size + Integer::from(1); //equiv to infinity
    let mut p_min_i : usize = 0;
    //find the minimum delimited plate
    //if, in the meantime, we identify that there is only three plates, we have found a square or rectangle and should return them
...
pub fn solve_restricted(size : Integer, squares : HashSet<Integer>) -> () {

    //println!{"Solving for size: {}", size};
    let mut config = RestrictedConfig::new(size, squares); //Creates the necessary starting plate.
    //println!("Config: {:?}", config);
    decompose_restricted( &mut config, 1);
